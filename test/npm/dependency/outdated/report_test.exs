defmodule NPM.Dependency.Outdated.ReportTest do
  ...
  alias NPM.Dependency.Outdated.Report
  ...
  describe "format_table" do
    test "formats header and rows" do
      output = Report.format_table(@packages)
    ...
    test "all up to date" do
      assert "All packages are up to date." = Report.format_table([])
    end
  ...
  describe "categorize" do
    test "groups by severity" do
      cat = Report.categorize(@packages)
  ...
  describe "summary" do
    ...
    test "all up to date" do
      assert "All packages are up to date." = Report.summary([])
    end
  end

  describe "security_risk" do
    test "finds major version packages" do
      risks = Report.security_risk(@packages)
    ...
    test "empty when no majors" do
      ...
      assert [] = Report.security_risk(packages)
    end
  end
end
