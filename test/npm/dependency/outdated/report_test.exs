defmodule NPM.Dependency.Outdated.ReportTest do
  ...
  describe "format_table" do
    test "formats header and rows" do
      output = NPM.Dependency.Outdated.Report.format_table(@packages)
    ...
    test "all up to date" do
      assert "All packages are up to date." = NPM.Dependency.Outdated.Report.format_table([])
    end
  ...
  describe "categorize" do
    test "groups by severity" do
      cat = NPM.Dependency.Outdated.Report.categorize(@packages)
  ...
  describe "summary" do
    test "generates summary" do
      sum = NPM.Dependency.Outdated.Report.summary(@packages)
    ...
    test "all up to date" do
      assert "All packages are up to date." = NPM.Dependency.Outdated.Report.summary([])
    end
  end

  describe "security_risk" do
    test "finds major version packages" do
      risks = NPM.Dependency.Outdated.Report.security_risk(@packages)
    ...
    test "empty when no majors" do
      ...
      assert [] = NPM.Dependency.Outdated.Report.security_risk(packages)
    end
  end
end
