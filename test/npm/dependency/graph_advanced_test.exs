defmodule NPM.Dependency.GraphAdvancedTest do
  ...
  describe "transitive_deps" do
    test "finds all reachable packages" do
      deps = NPM.Dependency.Graph.transitive_deps(@adj, "app")
    ...
    test "leaf has no transitive deps" do
      deps = NPM.Dependency.Graph.transitive_deps(@adj, "lodash")
  ...
  describe "shortest_path" do
    test "finds direct path" do
      path = NPM.Dependency.Graph.shortest_path(@adj, "app", "lodash")
      assert path == ["app", "lodash"]
    end

    test "finds indirect path" do
      path = NPM.Dependency.Graph.shortest_path(@adj, "app", "raw-body")
      assert path == ["app", "express", "body-parser", "raw-body"]
    end

    test "nil for unreachable" do
      assert nil == NPM.Dependency.Graph.shortest_path(@adj, "raw-body", "app")
    end

    test "same node path" do
      path = NPM.Dependency.Graph.shortest_path(@adj, "app", "app")
      assert path == ["app"]
    end
  end
  ...
  describe "max_depth" do
    test "computes max depth from root" do
      assert 3 = NPM.Dependency.Graph.max_depth(@adj, "app")
    end

    test "zero depth for leaf" do
      assert 0 = NPM.Dependency.Graph.max_depth(@adj, "lodash")
    end
  end

  describe "impact" do
    test "lodash has high impact" do
      score = NPM.Dependency.Graph.impact(@adj, "lodash")
      assert score >= 2
    end

    test "leaf with no dependents" do
      assert 0 = NPM.Dependency.Graph.impact(@adj, "app")
    end
  end
