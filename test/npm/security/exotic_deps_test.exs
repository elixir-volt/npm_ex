defmodule NPM.Security.ExoticDepsTest do
  ...
  test "detects exotic dependency specs" do
    assert ExoticDeps.exotic?("github:tanstack/router#abc")
    assert ExoticDeps.exotic?("git+https://github.com/user/repo.git")
    assert ExoticDeps.exotic?("https://example.com/pkg.tgz")
    assert ExoticDeps.exotic?("file:../local")
    assert ExoticDeps.exotic?("user/repo#abc")

    refute ExoticDeps.exotic?("^1.2.3")
    refute ExoticDeps.exotic?("latest")
    refute ExoticDeps.exotic?("npm:react@^18")
  end
  ...
  test "raises for transitive exotic optional dependencies by default" do
    ...
    assert_raise ExoticDeps.Error,
                 ~r/@tanstack\/setup: github:tanstack\/router/,
                 fn ->
                   ExoticDeps.validate!("@tanstack/history", "1.161.12", info)
                 end
  end

  test "blocks direct exotic dependencies unless allowed" do
    assert_raise ExoticDeps.Error, ~r/exotic direct dependency/, fn ->
      ExoticDeps.validate_direct!("local-pkg", "file:../local-pkg")
    end

    Application.put_env(:npm, :exotic_deps, ["file:../local-pkg"])

    assert :ok = ExoticDeps.validate_direct!("local-pkg", "file:../local-pkg")
  end
  ...
  test "can be disabled through env var" do
    ...
    assert :ok = ExoticDeps.validate!("pkg", "1.0.0", info)
  end
