defmodule NPM.Install.ScriptInstallTest do
  ...
  test "installs packages to content-addressed cache dir" do
    ...
    assert :ok = NPM.Install.ScriptInstall.install(deps, @test_opts)
    assert NPM.Install.ScriptInstall.installed?()

    nm = NPM.Install.ScriptInstall.node_modules_dir!()
  ...
  test "second call with different deps raises" do
    ...
    deps2 = %{"is-odd" => "^3.0.0"}
    assert :ok = NPM.Install.ScriptInstall.install(deps1, @test_opts)

    assert_raise Mix.Error, ~r/same dependencies/, fn ->
      NPM.Install.ScriptInstall.install(deps2, @test_opts)
    end
  end
  ...
  test "force reinstalls" do
    deps = %{"is-number" => "^7.0.0"}
    assert :ok = NPM.Install.ScriptInstall.install(deps, @test_opts)
    assert :ok = NPM.Install.ScriptInstall.install(deps, [force: true] ++ @test_opts)
  end

  test "install_dir! raises when not installed" do
    assert_raise Mix.Error, ~r/not been called/, fn ->
      NPM.Install.ScriptInstall.install_dir!()
    end
  end
end
